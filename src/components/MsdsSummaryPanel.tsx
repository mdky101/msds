...
function Summary({
  ...
  if (isEmpty) {
    return (
      <p className="bg-cloud text-charcoal mt-2.5 p-3 text-xs">
  ...
  return (
    <div className="bg-cloud mt-2.5 space-y-2.5 p-3">
          ...
          <ul className="mt-1.5 space-y-1">
            {summary.officialPictograms.map((code) => (
              <li key={code} className="text-xs">
                <span className="text-mute font-mono">{code}</span>{" "}
                <strong className="text-ink font-medium">
                  {GHS_CATALOG[code].name}
                </strong>
                <span className="text-charcoal"> — {GHS_CATALOG[code].meaning}</span>
              </li>
            ))}
          </ul>
          {hasDiff && photoPictograms.length > 0 && (
            <p className="border-hazard-warning text-charcoal mt-2.5 border-l-2 pl-2.5 text-[11px] leading-relaxed">
      ...
      {summary.sections.map((section) => (
        <details key={section.no} className="bg-canvas group">
          <summary className="text-ink flex cursor-pointer list-none items-center gap-1.5 px-2.5 py-2.5 text-xs font-medium select-none">
            <span className="text-mute font-mono">{section.no}항</span>
            ...
            <span
              aria-hidden
              className="text-mute ml-auto transition-transform group-open:rotate-90"
            >
              ›
            </span>
          </summary>
          <div className="border-hairline-soft space-y-2.5 border-t px-2.5 py-2.5">
            {section.groups.length === 0 ? (
              <p className="text-mute text-[11px]">등록된 내용이 없습니다.</p>
            ) : (
              section.groups.map((g) => (
                <div key={g.code}>
                  <p className="text-mute text-[11px] font-medium">{g.title}</p>
                  <ul className="mt-0.5 space-y-0.5">
                    {g.items.map((item, j) => (
                      <li
                        key={`${g.code}-${j}`}
                        className="text-charcoal flex gap-1.5 text-[11px] leading-relaxed"
                      >
